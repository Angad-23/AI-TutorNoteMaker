...
public class DashboardController {
    ...
    public String dashboard(@AuthenticationPrincipal UserDetails userDetails, Model model) {
        ...
        TutorUser tutor = dashboardService.getTutor(username);

        List<SessionNote> allNotes = dashboardService.getAllNotesForTutor(tutor);
        List<SessionNote> todayNotes = dashboardService.getTodayNotes(allNotes);
        ...
        List<SessionNote> recentNotes = dashboardService.getRecentNotes(allNotes, 5);

        model.addAttribute("username", username);
        model.addAttribute("fullName", tutor.getFullName());
        model.addAttribute("totalNotes", allNotes.size());
        model.addAttribute("todayNotes", todayNotes.size());
        model.addAttribute("totalStudents", uniqueStudents);
        model.addAttribute("recentNotes", recentNotes);
